  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="状态">
        <template slot-scope="scope">
          <span v-if="scope.row.status === -1">正在计算MD5 <span style="font-weight: bold; color: red;">{{MD5Percent}}</span></span>
  ...
  export default {
    ...
    methods: {
      ...
      filesAdded(up, files) {
        files.forEach((f) => {
          f.status = -1;
          FileMd5(f.getNative(), (e, md5,status,percent) => {
            f["md5"] = md5;
            f.status = status;
            // console.log(percent);
            this.MD5Percent = percent;
            // console.log(this.MD5Percent);
          });
        });
      },
      deleteFile(id) {
